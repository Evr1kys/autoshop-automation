...
from tgbot.utils import models, utils, utils
...
@adminRouter.callback_query(F.data == "clean_invalid_users")
async def clean_invalid_users(call: CallbackQuery, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    """Очистка невалидных пользователей из базы данных"""
    try:
        await call.message.edit_text("🧹 Начинаю очистку невалидных пользователей...")
        
        # Получаем общее количество пользователей до очистки
        all_users = await DB.all_users()
        total_before = len(all_users)
        
        # Очищаем невалидных пользователей
        await DB.clean_invalid_users()
        
        # Получаем количество пользователей после очистки
        valid_users = await DB.get_valid_users_for_mailing()
        total_after = len(valid_users)
        
        removed_count = total_before - total_after
        
        result_text = f"✅ Очистка завершена!\n\n"
        result_text += f"📊 Статистика:\n"
        result_text += f"• Пользователей до очистки: {total_before}\n"
        result_text += f"• Пользователей после очистки: {total_after}\n"
        result_text += f"• Удалено невалидных: {removed_count}"
        
        await call.message.edit_text(result_text, 
                                   reply_markup=BotButtons.ADMIN_INLINE.admin_panel(BotTexts).as_markup())
        
    except Exception as e:
        await call.message.edit_text(f"❌ Ошибка при очистке: {str(e)}", 
                                   reply_markup=BotButtons.ADMIN_INLINE.admin_panel(BotTexts).as_markup())
...
async def main_settings_edit(call: CallbackQuery, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    ...
    if action == "currency":
    ...
    elif action == "refill_commission_percent":
        await state.update_data(action=action)
        await call.message.edit_text(text=BotTexts.ADMIN_TEXTS.edit_main_setting.format(
            action=BotTexts.ADMIN_TEXTS.main_settings_values["refill_commission_percent"],
            value=settings.refill_commission_percent,
        ...
        if action == "faq":
            # Специальная обработка для FAQ с пагинацией
            faq_text = settings.__dict__[action] or "FAQ не установлен"
            pages = utils.split_text_to_pages(faq_text)
            
            if len(pages) == 1:
                # Если только одна страница, показываем обычно
                await call.message.edit_text(
                    text=BotTexts.ADMIN_TEXTS.edit_main_setting.format(
                ...
                # Несколько страниц - показываем первую с навигацией
                await call.message.edit_text(
                    text=BotTexts.ADMIN_TEXTS.edit_main_setting.format(
                        action=BotTexts.ADMIN_TEXTS.main_settings_values[action],
                        value=pages[0],
                    ),
                    reply_markup=BotButtons.ADMIN_INLINE.faq_pagination(BotTexts, 1, len(pages), action).as_markup(),
                    parse_mode="HTML"
                )
                # Сохраняем страницы в состоянии
                await state.update_data(faq_pages=pages, faq_action=action)
        else:
            await call.message.edit_text(text=BotTexts.ADMIN_TEXTS.edit_main_setting.format(
                action=BotTexts.ADMIN_TEXTS.main_settings_values[action],
...
async def enter_new_value_main_settings(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    ...
    if data['action'] == "ref_percent":
        ...
        value = int(msg.text)
    elif data['action'] == "refill_commission_percent":
...
@adminRouter.callback_query(F.data.startswith("faq_page:"))
async def faq_page_navigation(call: CallbackQuery, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    """
    Обработчик навигации по страницам FAQ
    """
    ...
    await call.message.edit_text(
        ...
        reply_markup=BotButtons.ADMIN_INLINE.faq_pagination(BotTexts, page, len(pages), action).as_markup(),
        parse_mode="HTML"
    )
...
@adminRouter.callback_query(F.data == "load_faq_from_file")
async def load_faq_from_file(call: CallbackQuery, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    """Загрузка FAQ из файла FAQ.md"""
    await state.clear()
    
    try:
        # Импортируем загрузчик FAQ
        from tgbot.utils.faq_loader import update_faq_from_file
        
        # Обновляем FAQ из файла
        success = await update_faq_from_file()
        
        if success:
            await call.answer("✅ FAQ успешно загружен из файла!", show_alert=True)
            # Возвращаемся к главным настройкам
            await call.message.edit_text(BotTexts.ADMIN_TEXTS.main_settings_text, 
                                       reply_markup=(await BotButtons.ADMIN_INLINE.main_settings(BotTexts)).as_markup())
        else:
            await call.answer("❌ Ошибка загрузки FAQ из файла", show_alert=True)
...
async def switchers_edit(call: CallbackQuery, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    ...
    old_value = settings.__dict__[action]
    new_value = values[old_value]
    
    await DB.update_settings(**{action: new_value})
    
    # Уведомление администратора об изменении настройки пополнений
    if action == "is_refill":
        status = "включены" if new_value else "выключены"
        message = f"✅ Пополнения {status}!\n\n⚠️ Важно: Пользователи увидят изменения только после:\n• Команды /start\n• Кнопки 'Главное меню'\n• Кнопки 'Назад' в главное меню"
...
async def enter_promo_uses(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    if msg.text.isdigit():
        ...
        await msg.reply(BotTexts.ADMIN_TEXTS.enter_amount_for_promo,
                        reply_markup=BotButtons.ADMIN_INLINE.custom_button(BotTexts, "extra_settings:promo_create").as_markup())
...
async def enter_promo_discount(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    if msg.text.isdigit() or msg.text.replace(".", "").isdigit():
        ...
        settings = await DB.get_settings()
        if settings.currency == models.Currencies.rub:
            promo_amount_rub = float(msg.text)
            promo_amount_usd = await utils.get_exchange(promo_amount_rub, 'RUB', 'USD', DB)
            promo_amount_eur = await utils.get_exchange(promo_amount_rub, 'RUB', 'EUR', DB)
        elif settings.currency == models.Currencies.usd:
            promo_amount_usd = int(msg.text)
            promo_amount_rub = await utils.get_exchange(promo_amount_usd, 'USD', 'RUB', DB)
            promo_amount_eur = await utils.get_exchange(promo_amount_usd, 'USD', 'EUR', DB)
        else:
            # eur
            promo_amount_eur = int(msg.text)
            promo_amount_usd = await utils.get_exchange(promo_amount_eur, 'EUR', 'USD', DB)
            promo_amount_rub = await utils.get_exchange(promo_amount_eur, 'EUR', 'RUB', DB)

        await DB.create_promocode(data['name'], data['uses'], promo_amount_rub, 
                                  promo_amount_usd, promo_amount_eur)
        ...
        await utils.send_admins("promo_is_created_alert", bot, DB,
...
async def enter_promo_name_for_delete(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    if await DB.get_promocode(name=msg.text):
        ...
        await utils.send_admins("promo_is_deleted_alert", bot, DB, username=msg.from_user.mention_html(), name=msg.text)
        await msg.answer(BotTexts.ADMIN_TEXTS.choose_action, 
                        reply_markup=(await BotButtons.ADMIN_INLINE.extra_settings_kb(BotTexts)).as_markup())
    else:
...
async def enter_new_number_of_refs_for_ref_lvl(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    if msg.text.isdigit():
        ...
        await utils.send_admins("edit_number_of_refs_for_ref_lvl_alert", bot, DB,
            username=msg.from_user.mention_html(),
            lvl=ref_lvl,
            count=msg.text,
            convert=utils.numeral_noun_declension(int(msg.text), BotTexts.TEXTS.person_s) 
...
async def enter_links_buttons_for_ad_button(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    ...
    await utils.send_admins("ad_button_is_created_alert", bot, DB,
        username=msg.from_user.mention_html(),
        name=data['name'],
    )
...
async def enter_name_for_delete_ad_button(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    ...
    await utils.send_admins("ad_button_is_deleted_alert", bot, DB,
        username=msg.from_user.mention_html(),
        name=msg.text,
    )
    await msg.answer(BotTexts.ADMIN_TEXTS.choose_action,
...
async def confirm_mail_start(call: CallbackQuery, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    ...
    blocked_users_count, not_found_users_count = 0, 0
    
    # Получаем только валидных пользователей для рассылки
    users = await DB.get_valid_users_for_mailing()
    if not users:
        await call.message.answer("❌ Нет валидных пользователей для рассылки!")
        await state.clear()
        return
        
    await utils.send_admins("mail_started_alert", bot, DB, username=call.from_user.mention_html())
    try:
        message: Message = (await state.get_data())['message']
        total_users = len(users)
        
        # Отправляем начальное сообщение о прогрессе для больших рассылок
        if total_users > 50:
            progress_msg = await call.message.answer(f"📤 Начинаю рассылку...\nПрогресс: 0/{total_users}")
        ...
        for i, user in enumerate(users, 1):
            ...
            try:
                ...
                # Небольшая задержка между отправками для избежания ограничений API
                await asyncio.sleep(0.05)
                
                # Обновляем прогресс каждые 10 пользователей или для больших рассылок
                if total_users > 50 and i % 10 == 0:
                    try:
                        await progress_msg.edit_text(f"📤 Рассылка в процессе...\nПрогресс: {i}/{total_users}")
                    except:
                        pass
            except (TelegramForbiddenError, TelegramBadRequest) as e:
                # Обрабатываем случаи когда пользователь заблокировал бота или чат не найден
                if "chat not found" in str(e) or "user not found" in str(e):
                    # Пользователь не существует или удален
                    not_found_users_count += 1
                    failed_users_count += 1
                    print(f"❌ Пользователь {user.user_id} не найден: {e}")
                elif "bot was blocked" in str(e) or "Forbidden" in str(e):
                    # Пользователь заблокировал бота
                    blocked_users_count += 1
                    failed_users_count += 1
                    print(f"🚫 Пользователь {user.user_id} заблокировал бота: {e}")
                else:
                    # Другие ошибки Telegram
                    failed_users_count += 1
                    print(f"⚠️ Ошибка при отправке пользователю {user.user_id}: {e}")
            except Exception as e:
                # Обрабатываем любые другие ошибки
                failed_users_count += 1
                print(f"💥 Неожиданная ошибка при отправке пользователю {user.user_id}: {e}")
                # Небольшая пауза при критических ошибках
                await asyncio.sleep(0.1)
    except:
        print_exc()
        await call.message.answer(BotTexts.ADMIN_TEXTS.mail_error)
    finally:
        # Удаляем сообщение о прогрессе если оно было создано
        if total_users > 50 and 'progress_msg' in locals():
            try:
        ...
        stats_text = BotTexts.ADMIN_TEXTS.success_mail_text.format(
        ...
        if blocked_users_count > 0 or not_found_users_count > 0:
            stats_text += f"\n\n📊 Детализация ошибок:"
            if blocked_users_count > 0:
                stats_text += f"\n🚫 Заблокировали бота: {blocked_users_count}"
            if not_found_users_count > 0:
                stats_text += f"\n❌ Пользователи не найдены: {not_found_users_count}"
...
async def enter_new_balance(msg: Message, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    if msg.text.isdigit or msg.text.replace(".", "").isdigit():
        ...
        settings = await DB.get_settings()
        if settings.currency == models.Currencies.rub:
            balance_rub = float(msg.text)
            balance_usd = await utils.get_exchange(float(msg.text), 'RUB', 'USD', DB)
            balance_eur = await utils.get_exchange(float(msg.text), 'RUB', 'EUR', DB)
        elif settings.currency == models.Currencies.usd:
            balance_usd = float(msg.text)
            balance_rub = await utils.get_exchange(float(msg.text), 'USD', 'RUB', DB)
            balance_eur = await utils.get_exchange(float(msg.text), 'USD', 'EUR', DB)
        else:
            # eur
            balance_eur = float(msg.text)
            balance_usd = await utils.get_exchange(float(msg.text), 'EUR', 'USD', DB)
            balance_rub = await utils.get_exchange(float(msg.text), 'EUR', 'RUB', DB)
        if data['action'] == "add_balance":
            await DB.update_user(data['user'].user_id, 
                                 balance_rub=data['user'].balance_rub + balance_rub,
        ...
        await utils.send_admins("new_balance_alert", bot, DB,
            username=msg.from_user.mention_html(),
    ...
    if refill:
        ...
        match settings.currency.value:
            case "rub":
                sum = refill.amount
            case "usd":
                sum = await utils.get_exchange(refill.amount, "RUB", "USD", DB)
            case "eur":
                sum = await utils.get_exchange(refill.amount, "RUB", "EUR", DB)
...
async def stats_open(call: CallbackQuery, state: FSMContext, BotTexts: BTs.Ru | BTs.En | BTs.Ua):
    ...
    for refill in all_refills:
        match settings.currency:
            ...
            case models.Currencies.usd:
                refill_amount = await utils.get_exchange(refill.amount, 'RUB', 'USD', DB)
            case models.Currencies.eur:
                refill_amount = await utils.get_exchange(refill.amount, 'RUB', 'EUR', DB)
